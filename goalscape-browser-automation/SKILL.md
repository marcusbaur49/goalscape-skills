...
Goalscape exposes a set of MCP tools that should be preferred over browser automation wherever possible. They are faster, more reliable, and return structured data (UUIDs, progress, importance) that can be used directly in subsequent operations.

### Available MCP Tools

| Tool | Purpose |
|---|---|
| `goalscape:get-project-overview` | Get full goal tree + persons + tags for a project. **Use this as the primary way to read project state.** Returns all goal IDs, names, importance, and progress in one call. |
| `goalscape:get-goal-tree` | Get the goal hierarchy for a project (lighter alternative to overview). |
| `goalscape:get-goal` | Get detailed info for a specific goal including notes, comments, tags, responsibles. |
| `goalscape:add-goal-note` | Add a note (with optional label) to a goal. Supports HTML content. |
| `goalscape:update-goal-note` | Update an existing note's content or label (requires noteId). |
| `goalscape:add-goal-comment` | Add a comment to a goal's comment thread. |
| `goalscape:get-goal-comments` | Retrieve all comments on a goal. |
| `goalscape:get-goal-notes` | Get all notes for a goal. |
| `goalscape:add-goals` | Add multiple subgoals at once under a parent (comma-separated names). |
| `goalscape:add-goal` | Add a single subgoal under a parent. |
| `goalscape:update-goal` | Update goal properties (name, progress, importance, dates, etc.). |
| `goalscape:delete-goal` | Delete a goal and all its subgoals. |
| `goalscape:move-goal` | Move a goal to a different parent or reorder among siblings. |
| `goalscape:list-projects` | List all projects for the current user. |
| `goalscape:get-project` | Get project details including persons and tags. |
| `goalscape:create-project` | Create a new project. |
| `goalscape:list-project-persons` | List persons defined in a project. |
| `goalscape:list-project-tags` | List tags defined in a project. |
| `goalscape:add-project-person` | Add a person to a project. |
| `goalscape:add-project-tag` | Add a tag to a project. |
| `goalscape:assign-goal-responsible` | Assign a responsible person to a goal. |
| `goalscape:assign-goal-tag` | Assign a tag to a goal. |

### When to Use MCP vs. Browser Automation

**Use MCP for:**
- Reading project structure and goal IDs → `get-project-overview`
- Adding subgoals → `add-goal` or `add-goals`
- Adding/updating notes → `add-goal-note`, `update-goal-note`
- Adding comments → `add-goal-comment`
- Updating progress → `update-goal`
- Any read operation (always prefer MCP)

**Use browser automation for:**
- Setting importance weights (lock/unlock workflow — not yet in MCP)
- Reordering goals via drag-and-drop
- Visual inspection / screenshots for user feedback
- Any action not yet covered by MCP tools

### Typical MCP Workflow

1. Call `goalscape:get-project-overview` with the projectId from the URL to get all goal IDs and the current tree
2. Identify the target goal ID(s) from the tree
3. Use the appropriate MCP tool to act on the goal
4. For operations that require browser automation (importance, reordering), fall back to the browser with the goal IDs already known

**Project ID from URL:** Extract from `https://goalscape.app/#/projects/{projectId}` or `https://devel.goalscape.app/#/projects/{projectId}`
...
2. **Comments** (chat bubble icon): Comment thread with formatting (bold, italic, lists, links, images, video). Can notify by email.
...
**Prefer MCP:** `goalscape:add-goal` or `goalscape:add-goals` (for multiple at once)
...
**Prefer MCP:** `goalscape:add-goal` with the same parent ID
...
### Rename a Goal
**Prefer MCP:** `goalscape:update-goal` with new name
...
### Set Importance
**Browser automation required** (not yet in MCP):
1. Select the goal
...
**Browser automation required** — Marcus performs this manually due to reliability issues.
...
### Set Progress
**Prefer MCP:** `goalscape:update-goal` with progress value (0–100)
...
**Prefer MCP:** `goalscape:add-goal-note` (supports HTML content and optional label)
...
### Add Comments
**Prefer MCP:** `goalscape:add-goal-comment`
...
### Set Due Date
**Prefer MCP:** `goalscape:update-goal` with startDate/endDate
...
**Prefer MCP:** `goalscape:assign-goal-responsible` (requires person ID from `list-project-persons`)
...
### Delete a Goal
**Prefer MCP:** `goalscape:delete-goal` (deletes goal and all subgoals — use with care)
...
**Prefer MCP:** `goalscape:move-goal` for repositioning
...
1. **Read with MCP first** — always call `goalscape:get-project-overview` at the start of a session to get all goal IDs. This avoids browser navigation and is far more reliable than reading the DOM.

2. **Use the Goal List** for precise selection when browser automation is needed — clicking segments in the radial diagram can be tricky since segments vary in size. The Goal List at the bottom-left is more reliable for selecting goals by name.

3. **Use right-click context menus** for most browser actions — they provide the most direct path to adding goals, subgoals, and setting colors.

4. **Use `read_page` or `find`** to locate specific UI elements when the layout changes.

5. **Wait after actions** — Goalscape sometimes has short animations. Add a brief wait (0.5–1s) after clicking to ensure the UI has updated.

6. **Check the URL** to understand which project and goal are currently selected.

7. **Importance is relative** — when the user says "make goal X more important", you need to increase its importance slider, understanding that sibling goals will automatically decrease.

8. **Clockwise = sequence** — goals arranged clockwise represent the order/sequence. This order can only be changed by dragging segments to a new position — there is no sort button.

9. **Progress propagation** — leaf goal progress is manual; parent goals auto-calculate from children weighted by importance.

10. **Double-click to drill in/out** — double-click a goal with subgoals to center it; double-click the center to go back up (only works if you've previously drilled in — at the root level there's nowhere to go up to). Goals without subgoals won't respond to double-click for centering.

11. **Unlimited depth** — goals can be broken down into as many levels as needed. Encourage users to break goals into smaller, more achievable pieces.

12. **Use the Undo button in the top bar** (left arrow icon) instead of ⌘Z when the Notes editor has focus, as keyboard undo may be captured by the text editor instead of the Goalscape app.

13. **Blue circle handle = importance** — The small blue circle at the outer edge of a selected segment is for adjusting importance by dragging. It is NOT for reordering goals. Be careful not to accidentally drag it.

14. **Always use `form_input` for renaming goals** — Never use `type` after creating or selecting a goal. Use `read_page(filter="interactive")` to find the goal name textbox in the right panel, then `form_input(ref=..., value="Name")`. Typing without a confirmed text input focus will inject characters into the SVG diagram.

15. **Use the lock/unlock workflow for setting multiple importance values** — When setting specific percentages on multiple siblings, set each value and lock it before moving to the next. Unlock all when done. See "Set Importance on Multiple Sibling Goals" above. Marcus performs this manually due to automation reliability issues.

16. **Froala editor injection** — For the Notes & Actions rich text editor, use: select `.fr-element.fr-view`, set `innerHTML`, dispatch `input` event with `bubbles: true`. Prefer `goalscape:add-goal-note` MCP tool instead.

17. **Goal selection for small/narrow goals** — Use Goal List panel for goals under ~5% importance; clicking diagram segments directly risks accidental goal creation.
